...
public class CsvToEdgeWeightedGraphConverter {
	...
	public static EdgeWeightedGraph generateEdgeWeightedGraph(String csvFile, String graphFile) {
		...
		while (in.hasNextLine()) {
			...
			if (!st.contains(lineSegment[0]))
				st.put(lineSegment[0], counter++);
			if (!st.contains(lineSegment[1]))
				st.put(lineSegment[1], counter++);
			linkedList.add(line);
			edges++;
		}
	...
	public static void main(String[] args) {
		EdgeWeightedGraph graph = generateEdgeWeightedGraph("src/cityGraph/CityConnections.csv",
				"src/cityGraph/Jingshis.txt");
